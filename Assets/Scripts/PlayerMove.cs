...
public class PlayerMove : MonoBehaviour
{
    ...
    void LoadHints()
    {
        ...
        var arrowList = transform.Find("Arrows");

        var windows = GameObject.FindGameObjectsWithTag("Window");
        foreach (var window in windows)
        {
            var newArrow = Instantiate(arrow, new Vector3(0, 0, 0), Quaternion.identity);

            var ac = newArrow.GetComponent<Arrow>();

            ac.window = window;

            ac.transform.SetParent(arrowList);
        }

        for (int i = 0; i < arrowList.childCount;i++)
        {
            var c = arrowList.GetChild(i);
           
                var delta = (i - arrowList.childCount / 2) / 5f;

                c.localPosition = new Vector3( delta ,1,  0f);
    ...
    void FixedUpdate()
    {
        ...
        if(level % 4 != 0){//verifica se o jogador n�o est� no terra�o
        ...
        if (Input.GetKey("z") && canJump) { //pula
            ...
            if (rb.velocity.y == 0)
            {
                ...
                rb.velocity = new Vector2(rb.velocity.x, maxJumpFrames);
    ...
    void OnGUI(){
        ...
        GUI.Label(new Rect(10,50,100,20),"Time left: "+secondsLeft.ToString(),style);
        GUI.Label(new Rect(10,90,100,20),"Score: "+stageScore.ToString(),style);
    }
}
